...
class eBayIntegration:
    ...
    def get_active_listings(self) -> List[Dict[str, Any]]:
        ...
        offers_data = self._paginated_get(offer_url, offer_params, "offers")
        published_offers = [o for o in offers_data if o.get("status") == "PUBLISHED"]
        ...
        inventory_items = self._paginated_get(inventory_url, {"limit": 100}, "inventoryItems")
    ...
    def _paginated_get(self, base_url: str, initial_params: Dict, key: str) -> List[Dict]:
        """Generic helper to fetch all pages from a paginated eBay endpoint."""
        results = []
        next_url = base_url
        ...
        try:
            while next_url:
                ...
                if response.status_code != 200:
                    print(f"Warning: API call to {next_url} failed with status {response.status_code}")
                    break
            ...
            print(f"Exception during paginated fetch from {base_url}: {str(e)}")
    ...
    def _join_offer_inventory(self, offers: List[Dict], inventory_map: Dict) -> List[Dict[str, Any]]:
        ...
        for offer in offers:
            ...
            sku = offer.get("sku")
            inventory_item = inventory_map.get(sku) or {}
            ...
            title = (offer.get("listing") or {}).get("title") or product.get("title") or sku or "Unknown Item"

            # Robust price extraction
            price_val = (offer.get("pricingSummary") or {}).get("price") or {}
            try:
                price = float(price_val.get("value") or 0.0)
